...
inline Cvec2 getScreenSpaceCoord(const Cvec3 &p, const Matrix4 &projection,
    ...
    if (p[2] > -CS175_EPS) {
        std::cerr
            << "WARNING: getScreenSpaceCoord of a point near or behind Z=0 "
               "plane. Returning screen-center instead."
            << std::endl;
...
inline double getScreenToEyeScale(double z, double frustFovY,
    ...
    if (z > -CS175_EPS) {
        std::cerr
            << "WARNING: getScreenToEyeScale on z near or behind Z=0 plane. "
               "Returning 1 instead."
            << std::endl;
        return 1;
    }
